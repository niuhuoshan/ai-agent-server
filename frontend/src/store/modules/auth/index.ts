...
import { fetchGetUserInfo, fetchLogin, fetchSmsLogin } from '@/service/api';
...
export const useAuthStore = defineStore(SetupStoreId.Auth, () => {
  ...
  async function loginBySms(phoneNumber: string, smsCode: string, redirect = true) {
    startLoading();
    const { data: loginToken, error } = await fetchSmsLogin(phoneNumber, smsCode);
    if (!error) {
      const pass = await loginByToken(loginToken);
      if (pass) {
        const isClear = checkTabClear();
