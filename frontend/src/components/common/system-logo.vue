...
import type { PublicPlatformConfiguration } from '@/service/api';
import {
  platformConfigurationUpdateEvent,
  readPublicPlatformConfiguration
} from '@/utils/platform-configuration';

const customLogo = ref(readPublicPlatformConfiguration()?.logoUrl || '');
const customLogoFailed = ref(false);
const logoInstanceId = useId().replaceAll(':', '-');

function gradientId(name: string) {
  return `system-logo-${logoInstanceId}-${name}`;
}

function refreshLogo(event?: Event) {
  const configured = event instanceof CustomEvent
    ? (event.detail as PublicPlatformConfiguration | undefined)
    : readPublicPlatformConfiguration();
  customLogo.value = configured?.logoUrl || '';
  customLogoFailed.value = false;
}

onMounted(() => window.addEventListener(platformConfigurationUpdateEvent(), refreshLogo));
onBeforeUnmount(() => window.removeEventListener(platformConfigurationUpdateEvent(), refreshLogo));
</script>

<template>
  <div class="app-logo">
    <img
      v-if="customLogo && !customLogoFailed"
      class="custom-logo"
      :src="customLogo"
      alt=""
