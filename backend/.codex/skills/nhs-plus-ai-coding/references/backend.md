...
- `nhs-modules/nhs-gen/src/main/resources/fm/java/*.ftl`
- `nhs-modules/nhs-demo/...`
- `nhs-modules/nhs-system/...`
- `nhs-modules/nhs-workflow/...`
- `nhs-common/nhs-common-mybatis/...`
...
- 除非所在模块明显另有约定，否则实体类继承 `group.aitools.nhs.common.mybatis.core.domain.BaseEntity`。
...
- `nhs-system` 的用户、角色、菜单、部门等模块常带数据权限、MPJ 联表、角色状态过滤，修改前先读对应 mapper/service。
...
- `nhs-workflow` 通常带 `@ConditionalOnEnable`，新增 workflow bean、controller、service 时检查同包是否需要该条件。
...
- 不要把 `nhs-system` 这类复杂逻辑强行简化成生成器式单表 CRUD。
